...
import { QAppCfgInputLLMAPI } from '@shared/qApp/cfgTypes'
...
const DsnInputLLMAPI: QAppDesignComponent<'InputLLMAPI'> = ({
  workflow,
  objectInfos,
  id,
  value,
  setValue,
  onDelete
}: QAppDesignProps<'InputLLMAPI'>) => {
  const { label, InputLabel } = useInputLabel(value?.label, id, 'InputLLMAPI', onDelete)
  ...
  useEffect(() => {
    if (seperateSlots) {
      ...
      const cfg: QAppCfgInputLLMAPI = {
        label,
        component: 'InputLLMAPI',
        seperateSlots,
        modelNameSlot,
        baseUrlSlot,
        apiKeySlot,
...
DsnInputLLMAPI.displayName = 'QAppDsnInputLLMAPI'

export default DsnInputLLMAPI
