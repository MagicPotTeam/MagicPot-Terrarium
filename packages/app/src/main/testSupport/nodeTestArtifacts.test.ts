...
describe('nodeTestArtifacts', () => {
  it('keeps disposable node test artifacts under desktop .magicpot-trash/MagicPot/<run-id>', async () => {
    const { resolveNodeTestArtifactPath, resolveNodeTestArtifactRoot } =
      await import('./nodeTestArtifacts')

    expect(resolveNodeTestArtifactRoot()).toMatch(
      /[\\/]\.magicpot-trash[\\/]MagicPot[\\/]run-[^\\/]+$/
    )
  ...
  it('honors the standardized desktop override for node-only test artifacts', async () => {
    ...
    expect(root).toMatch(
      /^D:[\\/]+Redirected[\\/]+Desktop[\\/]+\.magicpot-trash[\\/]+MagicPot[\\/]+[^\\/]+$/
    )
  })
})
