import type { MagicAgentActorRef } from './envelope.js'
...
export const minimumRisk = (kind: string): EffectivePolicyRisk | undefined => {
  ...
  if (kind === 'tool.invoke') return 'high'
  if (
    [
      'filesystem.write',
      'process.execute',
...
export const createTerminalPolicyRequest = (input: TerminalPolicyRequestInput): PolicyRequest => {
  const { command, args = [], cwd, confirm, ...context } = input
  ...
  return factory('assistant', context, {
