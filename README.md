...
再分发受 Microsoft Software License Terms for Visual Studio / Visual C++ Redistributable 约束。若维护者决定在 release artifact 中随包附带该安装器，需要先确认授权条件；公开源码仓不要直接携带该 Microsoft 二进制文件。当前 Windows embedded 打包配置会在维护者环境中查找 `vendor/windows/VC_redist.x64.exe` 作为可选安装器资源。
